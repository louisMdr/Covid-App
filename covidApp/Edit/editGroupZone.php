...
<html>
    <head>
        <title>C19PHCS - Edit Group Zone</title>
		<link rel="stylesheet" href="../style.css">
</head>

<?php include('../Employee/dropdownmenu.html');?>

<form action="editGroupZone.php" method="POST">
		<br><br>
		<div class="formDiv">
			<label for="address"><h3 class="instruction">Please enter the Group Zone's ID:</h3><br></label>
			<input type="text" name="group_id" required <?php echo isset($_POST['group_id']) ? 'value="' . $_POST['group_id'] . '"'  : 'value=""'; ?>>
		</div>
		<br>
	...
	$cols = $db->prepare('DESCRIBE comp353.groupzone');
	$cols->execute();
	...
	if(isset($_POST['searchEdit_GZ']) && count($_POST) > 2)
	{
		$insertStr = 'UPDATE comp353.groupzone SET ';
		...
		$insertStr = $insertStr . " WHERE Group_ID = :group_id";
		$addPsn = $db->prepare($insertStr);
		$addPsn->bindParam(':group_id', $_POST['group_id']);
	...
	else if(!empty($_POST['group_id']) && isset($_POST['searchEdit_GZ']))
	{
		//Not * so order is specific
		$person = $db->prepare('SELECT * FROM comp353.groupzone WHERE Group_ID = :group_id AND isDeleted = 1');
		$person->bindParam(':group_id', $_POST['group_id']);
		$person->execute();
		//to get index type
		$results = $person->fetch(PDO::FETCH_BOTH);
		if(is_array($results) && count($results) > 0)
		{
			echo "<br><h3 class=\"edit\">" . $results['Group_Name'] . "'s Current Information:</h3><br>
		<table border='1'>
	<thead>
	    <tr>";
		...
		{
			echo "<h3 class=\"instruction\">None with the Group ID: \"" . $_POST['group_id'] . "\" were found.</h3>";
		}
	}	
	?>

		<input type="submit" name="searchEdit_GZ" value="Edit" />
	</form>
</body>
</html>
